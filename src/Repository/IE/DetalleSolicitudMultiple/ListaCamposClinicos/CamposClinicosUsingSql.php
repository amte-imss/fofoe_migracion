...
use App\Repository\IE\DetalleSolicitud\ListaCamposClinicos\Carrera;
use App\Repository\IE\DetalleSolicitud\ListaCamposClinicos\CicloAcademico;
use App\Repository\IE\DetalleSolicitud\ListaCamposClinicos\Convenio;
use App\Repository\IE\DetalleSolicitud\ListaCamposClinicos\NivelAcademico;
...
final class CamposClinicosUsingSql implements CamposClinicos
{
    ...
    public function listaCamposClinicosBySolicitud(SolicitudId $solicitudId): array
    {
        $records = $this->connection->executeQuery('
            SELECT campo_clinico.id       AS id_campo_clinico,
                   ...
                   CASE WHEN estatus_campo.nombre IS NULL THEN \'Pendiente de pago\' ELSE estatus_campo.nombre END AS estatus,
                   campo_clinico.referencia_bancaria,
                   (
                       SELECT pago.id
                       FROM pago
                   ...
                   (
                       SELECT factura.id
                       FROM pago
                           JOIN factura ON pago.factura_id = factura.id
                       WHERE pago.solicitud_id = solicitud.id
                         AND pago.referencia_bancaria = campo_clinico.referencia_bancaria
                       LIMIT 1
                   ) AS factura_id,
            ...
            FROM campo_clinico
                ...
                LEFT JOIN nivel_academico ON carrera.nivel_academico_id = nivel_academico.id
                JOIN ciclo_academico ON campo_clinico.ciclo_academico_id = ciclo_academico.id
                JOIN unidad        ON campo_clinico.unidad_id       = unidad.id
                JOIN delegacion    ON unidad.delegacion_id          = delegacion.id
                LEFT JOIN estatus_campo ON campo_clinico.estatus_campo_id = estatus_campo.id
                LEFT JOIN trabajador_imss ON campo_clinico.id       = trabajador_imss.campo_clinico_id
            WHERE solicitud.id = :id
            GROUP BY
                solicitud.id, campo_clinico.id, carrera.id, estatus_campo.id,
        ...
        return array_map(fn(array $record) => new CampoClinico(
            ...
            $record['horario'],
            $record['asignatura'],
            new Pago($record['id_pago'], $record['factura_id'], $record['requiere_factura']),
            $record['estatus'],
    ...
    private function getNoSemanas(string $fechaInicial, string $fechaFinal): int
    {
        ...
        $semanas = $final->diffInWeeks($inicial);
